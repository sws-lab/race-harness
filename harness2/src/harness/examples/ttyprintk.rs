...
pub struct TtyPrintkExample {
    num_of_clients: usize
}

#[allow(dead_code)]
pub struct TtyPrintkModel {
...
impl TtyPrintkExample {
    pub fn new(num_of_clients: usize) -> TtyPrintkExample {
        TtyPrintkExample { num_of_clients }
    }
}
...
impl HarnessExample for TtyPrintkExample {
    ...
    fn build_model(&self, context: &mut StateMachineContext, process_set: &mut ProcessSet) -> Result<Self::Model, HarnessError> {
        ...
        let tty_clients = (0..self.num_of_clients).map(| client_id |
