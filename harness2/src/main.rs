...
fn main() {
    ...
    if harness_context.is_executable() {
        ...
        codegen.format(&mut codegen_output, &context, &process_set, harness_build.get_template(), control_flow_nodes.iter().map(| (process, node) | (*process, node)), mutex_set.get_mutexes()).unwrap();
    } else {
