...
pushd linux-6.14.9
patch -p0 < ../1/patches/linux-6.14.9-minimal.patch
popd

pushd linux-kernel-goblint 
find rh_examples -name "*.json" -print | while read TASK_FILEPATH; do
        ...
        ./driver.py --db ../linux-6.14.9.db \
                --goblint $PWD/../linux-verification-goblint/goblint \
...
pushd linux-6.14.9
patch -p0 < ../1/patches/linux-6.14.9-seeded.patch
popd
